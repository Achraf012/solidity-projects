...
describe("simple test for my NFT code", function () {
    ...
    it("Ensure only the owner can transfer.", async function () {
        await contract.connect(user2).mint("achraf");
        await expect(contract.connect(user1).transferNFT(user1.address, 0)).to.be.revertedWith("Not owner or approved")
    ...
    it(" Owner can approve someone", async function () {
        await contract.connect(user1).mint("achraf");
        await contract.connect(user1).approve(user2, 0)
        expect(await contract.getApprovedAccounts(0)).to.equal(user2.address)
    })
    it("Only the owner should be able to approve someone", async function () {
        await contract.connect(user1).mint("achraf");
        await expect(contract.connect(user2).approve(user2, 0)).to.be.revertedWith("Not the owner")
    })
    it("Approval resets after transfer", async function () {
        const ZERO_ADDRESS = ethers.ZeroAddress;
        await contract.connect(user1).mint("achraf");
        await contract.connect(user1).approve(user2, 0)
        await contract.connect(user2).transferNFT(user2, 0)
        expect(await contract.getApprovedAccounts(0)).to.equal(ZERO_ADDRESS);
    })
});